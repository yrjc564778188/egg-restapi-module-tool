...
class TableForm extends Component {
	...
	componentDidMount() {
		...
		if (id) {
			dispatch({ type: "userFillForm/loadTable", payload: { id, ...Const } });
		}
	}

	componentWillUnmount() {
		this.props.dispatch({
			type: "userFillForm/resetState"
		});
	}
...
export default connect(({ userFillForm, app }) => {
	return {
		...userFillForm,
		content: userFillForm.con,
		uid: app.user.uid,
		name: app.user.name
	};
})(TableForm);
